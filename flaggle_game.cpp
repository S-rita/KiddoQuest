...
Flaggle_game::Flaggle_game(Members &member, int index, std::vector<Country>& VecCountry, QWidget *parent) :
    QMainWindow(parent),
    ...
    VecCountry(VecCountry)
{
    ...
    for (int i = 0; i < VecCountry.size(); ++i) {
        ui->guessFlagcomboBox->addItem(QString::fromStdString(VecCountry[i].getCountryName()));
    }

    srand((unsigned) time(NULL));
    flag = VecCountry[rand()%(VecCountry.size())];
    QPixmap flagpic(QString::fromStdString(flag.getPathFlag()));
    ui->flag->setPixmap(flagpic.scaled(ui->flag->size(), Qt::IgnoreAspectRatio));
    QPixmap picgray(":/image for c++ project/Background/gray.png");
    ...
    for (Country i: VecCountry) {
        if (i.getCountryName() == ans) {
            ...
            if (flag.getCountryName() == ans) {
                ...
                FlaggleWingamecomplete.setTime(playtime);
                FlaggleWingamecomplete.exec();
                Flaggle_game::close();
                RoundGame = 9;
                totalScore = 10;
                showPic(true);
                ...
                FlaggleLosegamecomplete.setTime(playtime);
                FlaggleLosegamecomplete.lose();
                FlaggleLosegamecomplete.exec();
                Flaggle_game::close();
                RoundGame = 9;
                totalScore = 10;
                GeographyWindow *geographyWindow = new GeographyWindow(member, index, this);
