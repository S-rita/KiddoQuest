#include "guesscountry_game.h"
#include "./ui_guesscountry_game.h"
#include "country.h"
...
GuessCountry_game::GuessCountry_game(QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::GuessCountry_game)
{
    ui->setupUi(this);
    timer.start();
    ...
    ui->guessCountrycomboBox->setEditable(true);

    for (int i = 0; i < VecMap.size(); ++i) {
        ui->guessCountrycomboBox->addItem(QString::fromStdString(VecMap[i].getCountryName()));
    }
...
GuessCountry_game::~GuessCountry_game()
{
    delete ui;
}

void GuessCountry_game::on_pushButton_clicked()
{
    bool found = false;
    std::string ans = ui->guessCountrycomboBox->currentText().toStdString();
...
QPixmap GuessCountry_game::showPic(bool check) {
    QPixmap green(QString::fromStdString("C:/n/year1/Cpp/pj/pic/green.png"));
    QPixmap gray(QString::fromStdString("C:/n/year1/Cpp/pj/pic/gray.png"));
    if (check) {
        return green;
    } else {
        return gray;
